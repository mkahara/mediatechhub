...
class EPKB_Manage_KB_Page {
	...
	function __construct() {
		...
		add_action( 'eckb_manage_content_export_tab_body', array( $this, 'display_export_tab' ), 10, 2 );
		add_action( 'eckb_manage_content_import_tab_body', array( $this, 'display_import_tab' ), 10, 2 );
		...
		foreach ( $tabs as $tab ) { 
			...
			<div class="epkb-manage-tabs__button <?php echo $active_class; ?>">
				...
				<a class="epkb-manage-tabs__button__title" href="<?php echo $tab['url']; ?>" data-kb_id="<?php echo $tab['kb_id']; ?>"
				   data-target="<?php echo $tab['target']; ?>"><?php echo $tab['title']; ?></a><?php
		...
		<div class="epkb-manage-content  <?php echo $active_class; ?>" id="kb_<?php echo $kb_id; ?>">
			...
			<div class="epkb-manage-content__tabs"><?php
				...
				<div id="kb_<?php echo $kb_id; ?>_manage" class="epkb-manage-content__tab <?php echo $active; ?>"><?php
					...
					<div class="epkb-admin-row">						<?php
						$HTML->info_box_v2(
							'epkbfa epkbfa-graduation-cap',
							'Status',
							'<span class="epkb-manage-kb-status"><i class="'.$icon.'"></i>'.ucfirst($kb_config['status']).'</span>',
							'View this KB&nbsp;&nbsp;<i class="ep_font_icon_external_link"></i>',
							$link );						?>
					</div>
	...
	public function display_import_tab( $kb_id, $kb_config ) {	?>
		...
		<div class="epkb-admin-info-box">
			...
			<div class="epkb-admin-info-box__body">
				...
				<ul>
					<li>Test import and export on your staging or test site before importing configuration in production.</li>
					<li>Always back up your database before starting the import.</li>
					<li>Preferably run import outside of business hours.</li>
				</ul>
	...
	public function display_export_tab( $kb_id, $kb_config ){
		...
		$HTML->notification_box_basic( array(
				...
				'desc'      =>
					'
						<p>A Beta phase generally begins when the software feature is complete but likely to contain a number of known or unknown bugs.</p>
